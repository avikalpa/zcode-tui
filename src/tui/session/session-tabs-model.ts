// Ported from opencode v2.0.7 packages/tui/src/context/session-tabs-model.ts
// — the session-tabs state machine, verbatim: open/close/reorder, the
// closed-tab stack (limit 10, reopen walks it and restores positions), the
// bounded in-memory switch history (limit 100), cycle arithmetic, and the
// adaptive strip layout (width clamp 8/22/32 with ‹N / N› overflow
// markers). Not ported: seedSessionTabMotion (it exists only to feed the
// upstream animation framework this binding does not have) and
// sessionTabDetail (project/branch decoration from their data stack).
