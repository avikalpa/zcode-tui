...
// (tui/routes/session/activity-summary.ts) — the low-verbosity activity
// group's summary row: "3 commands, 1 edit, 2 thoughts, 4 reads". Upstream
// types messages through @opencode/client; ours are the rows layer's
// NormMessage / NormPart, so the message accessor returns NormMessage and
// tool names are lowercased before the canonical map (our host names tools
// in upper case). #51131 (TRANSCRIPT-VERBOSITY), the R52 wave.
...
import { resolvePart, type NormMessage, type NormPart } from "./rows";

type ActivityPart = Extract<NormPart, { type: "reasoning" } | { type: "tool" }>;
...
export function busyLabel(part: ActivityPart) {
  if (part.type === "reasoning") return "Thinking…"
  const name = canonicalToolName(part.name.toLowerCase())
...
export function activitySummary(items: readonly Item[], instructions: number, closed = false) {
  ...
  items.forEach((item) => {
    if (item.part.type === "reasoning") {
      ...
      if (!isActive(item, closed) && reasoningText(item.part)) counts.thought++
      return
    }
    const name = canonicalToolName(item.part.name.toLowerCase())
    if (name === "execute") {
      ...
      calls.forEach((call) => {
        if (canonicalToolName(call.tool.toLowerCase()) === "read") counts.read++
        else counts.tool++
      })
...
function failed(item: Item) {
  ...
  if (canonicalToolName(item.part.name.toLowerCase()) !== "execute" || item.part.state.status === "streaming")
    return false
...
function reasoningText(part: Extract<NormPart, { type: "reasoning" }>) {
