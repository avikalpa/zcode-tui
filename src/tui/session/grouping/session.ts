...
// (tui/routes/session/grouping/session.ts) — the session projection over the
// grouping tree: entry/node/group shapes, history-batch hydration, live
// append/complete, pending-partition for permission-blocked tools.
// One wiring seam (marked below): upstream types CacheUsage.model through
// @opencode/client; our protocol carries no typed model yet — slice 2's rows
// layer re-homes it onto the real shape. Slice 1 of the transcript-render
// refactor family: model only, unwired.
import { groupEntries, mergeGroups, splitGroups, type GroupNode } from "./tree"
import type { NormMessage } from "../rows"
...
export type GroupKind = "reasoning" | "exploration" | "activity" | "instructions"
 ...
 */
export function partPath(part: AppendPart, verbosity: Verbosity = defaultVerbosity): readonly GroupKind[] {
...
export function messagePath(message: NormMessage, verbosity: Verbosity = defaultVerbosity): readonly GroupKind[] {
...
export function instructionPaths(message: NormMessage | undefined): string[] {
  if (message?.type !== "synthetic") return []
  // Our protocol carries no synthetic instruction metadata today, so this
  // stays inert on live data; the shape follows upstream metadata.instruction.
  const instruction = (message.meta as { instruction?: { paths?: unknown } } | undefined)?.instruction
